...
use crate::constants::{MAX_PAGES, PAGE_SIZE, TABLES_PAGES_FIRST_BYTE};
...
fn init_pages(file: &mut File) -> Result<HashMap<usize, [u8; 4096]>, String> {
    ...
    for i in 0..MAX_PAGES {
        file.seek(SeekFrom::Start((PAGE_SIZE * i).try_into().unwrap()))
            .unwrap();
...
impl Pager {
    ...
    fn flush_page(&mut self) {
        ...
        self.file
            .seek(SeekFrom::Start(
                ((self.current_page * PAGE_SIZE) + self.page_cursor)
                    .try_into()
                    .unwrap(),
            ))
            .unwrap();
        self.file
            .write_all(self.pages.get(&self.current_page).unwrap())
            .unwrap();
    }
    ...
    pub fn add_table(&mut self, table_string: String) {
        ...
        if self.pages.get(&self.current_page).unwrap() == &[0; PAGE_SIZE] {
            ...
            self.file
                .seek(SeekFrom::Start(
                    (self.current_page * PAGE_SIZE).try_into().unwrap(),
                ))
                .unwrap();
            ...
            'outer: loop {
                for i in 0..PAGE_SIZE - 1 {
                    ...
                    if self.pages.get(&self.current_page).unwrap()[self.page_cursor] == 0 {
                        let arr: &[u8] =
                            &self.pages.get(&self.current_page).unwrap()[self.page_cursor..];
                        let len = arr.len();
    ...
    fn write_new_page(&mut self, position: usize) -> Result<(), String> {
        self.file
            .seek(SeekFrom::Start((position * PAGE_SIZE).try_into().unwrap()))
            .unwrap();
        self.file.write_all(&[255; PAGE_SIZE]).unwrap();
